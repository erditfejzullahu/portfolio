...
const SideProjects = () => {
  ...
  return (
    <>
      ...
      {hoveredProject && (
        <div onMouseMove={handleMouseMove} onMouseLeave={() => {setTransformStyle("")}} style={{transform: transformStyle}} ref={bottom} className={`flex will-change-transform flex-col transition-all justify-between fixed bottom-0 my-auto top-0 ${unHovered ? "left-[120px]" : "left-[160px]"} max-[1820px]:left-0 max-[1820px]:right-0 max-[1820px]:mx-auto max-[1820px]:max-w-[600px]  max-[1820px]:bottom-auto   ${isTopbarVisible ? "max-[1820]:top-[220px]" : "max-[1820px]:top-[140px]"}  transition-all ease-in max-w-[300px] rounded-xl p-4 max-h-[32%] h-full bg-white shadow-lg shadow-gray-500 animate-fadeIn ${addCloser ? "animate-fadeOutLeft" : "animate-fadeInRight"} z-[90] overflow-y-auto overflow-x-hidden!`}>
