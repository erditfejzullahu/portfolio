...
const SideProjects = () => {
  ...
  return (
    <>
      <div ref={top} className="absolute  left-4 top-0 bottom-0 max-h-[30%] overflow-y-scroll my-auto rounded-xl shadow-[0_0_30px_rgba(0,0,0,0.2)] p-3 bg-white flex flex-col gap-4 items-center">
        {projects.map((item) => (
          <div onMouseEnter={() => mouseEnter(item)} onMouseLeave={() => setUnHovered(true)} key={item.id} className="cursor-pointer interactive relative flex items-center justify-center size-16 rounded-lg hover:w-24 transition-all ease-linear shadow-[0_0_30px_rgba(0,0,0,0.2)]">
            <Image src={item.image} alt={"project"} className="object-cover rounded-lg size-16 hover:w-24 hover:h-20 transition-all" />
          </div>
        ))}
      </div>
      {hoveredProject && (
        <div ref={bottom} className={`flex flex-col justify-between absolute bottom-0 my-auto top-0 ${unHovered ? "left-[120px]" : "left-[160px]"} transition-all ease-in max-w-[300px] rounded-xl p-4 max-h-[32%] h-full bg-white shadow-[0_0_30px_rgba(0,0,0,0.2)] animate-fadeIn ${addCloser ? "animate-fadeOutLeft" : "animate-fadeInRight"}`}>
